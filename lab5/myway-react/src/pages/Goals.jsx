...
import { addGoal, getGoals,updateGoalStatus,deleteGoal,updateCompletedSteps,updateUserCoins} from "../firebase/firestoreService";
import { useAuth } from "../firebase/AuthContext";
import { markGoalCompleted } from "../api";
  ...
  const handleUpdateStatus = async (goal, newStatus) => {
    ...
    try {
      if (newStatus === "completed") {
        await markGoalCompleted(user.uid, goal.id);
        const reward = goal.steps.length > 0 ? goal.steps.length * 15 : 20;
        await updateUserCoins(user.uid, reward);
      } else {
